"""
XGBoost experiment for SARGEN pattern detection
...
mlflow.set_experiment("sargen_pattern_detection_xgboost")
...
# Create labels (heuristic: top 1.5% by total_amount + high txn_count as suspicious)
amount_threshold = account_features['total_amount'].quantile(0.985)
txn_threshold = account_features['txn_count'].quantile(0.95)
account_features['is_suspicious'] = (
    (account_features['total_amount'] >= amount_threshold) | 
    (account_features['txn_count'] >= txn_threshold)
).astype(int)
...
with mlflow.start_run(run_name="xgboost_v1"):
    ...
    mlflow.sklearn.log_model(model, "xgboost_model", 
                            registered_model_name="sargen_xgboost")
